...
def generate_launch_description():
    ...
    package_name = "robot_cafe_sim"
    # Launch and config file paths
    gazebo_launch_file_path = "launch/robot_launch.launch.py"
    ekf_launch_file_path = "launch/ekf_launch.launch.py"
    ekf_config_file_path = "config/ekf.yaml"
    map_file_path = "maps/cafe_world_map.yaml"
    nav2_params_path = "config/navigation_parameters.yaml"
    rviz_config_file_path = "rviz/nav2_default_view.rviz"

    # Set the path to different packages
    pkg_share_gazebo = FindPackageShare(package=package_name).find(package_name)
    pkg_share_localization = FindPackageShare(package=package_name).find(package_name)
    pkg_share_navigation = FindPackageShare(package=package_name).find(package_name)
